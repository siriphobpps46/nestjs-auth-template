...
#### 4. การจัดการพอร์ตและการเชื่อมต่อ Cross-Origin (.env & CORS)
* **ปัญหา**: การรันแอปพลิเคชัน Next.js (Frontend) ร่วมกับ NestJS (Backend) บนเครื่องเดียวกัน หากกำหนดให้ทั้งคู่ใช้พอร์ตเริ่มต้นเดียวกัน (เช่น 3000) จะเกิดการชนกันของพอร์ต (`EADDRINUSE`) ทำให้เซิร์ฟเวอร์ใดเซิร์ฟเวอร์หนึ่งล่มทันที และหากไม่มีการเปิด CORS ในฝั่ง Backend จะทำให้เบราว์เซอร์สกัดกั้นการเชื่อมต่อทั้งหมดจาก Frontend
* **การแก้ไข**: 
  - แยกพอร์ตพัฒนาอย่างชัดเจน โดยใช้พอร์ต `3000` สำหรับ Next.js (Frontend) และพอร์ต `3001` สำหรับ NestJS (Backend) ผ่านการตั้งค่า `PORT` ในไฟล์ `.env`
  - ใช้ตัวแปรสภาพแวดล้อมมาตรฐานสำหรับ Next.js โดยใช้พรีฟิกซ์ `NEXT_PUBLIC_API_URL="http://localhost:3001"` เพื่อให้ฝั่ง Client ดึงค่าไปใช้ได้ในระดับรันไทม์
  - เปิดใช้งาน CORS ใน NestJS เสมอด้วยคำสั่ง `app.enableCors()` ในไฟล์ `src/main.ts` เพื่อยินยอมให้แอปพลิเคชันที่รันอยู่คนละพอร์ตสามารถสื่อสารข้ามโดเมนได้อย่างปลอดภัย
